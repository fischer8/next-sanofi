  ...
  const allQuestions = questions[menuPage].map(({ id, ask, value }) => {
    ...
    return (
      <li className="mb-1 md:mb-0 md:hover:border-gray-400 md:hover:bg-purple-200 rounded border flex flex-row justify-between" key={id}>
        <input onChange={() => handleScore({ id, value })} className="appearance-none border-gray-300 hover:border-gray-400 rounded-l border-e w-1/12 cursor-pointer me-px md:me-2 checked:bg-purple-500" checked={isChecked} id={`inp-${id}`} type="checkbox" />
